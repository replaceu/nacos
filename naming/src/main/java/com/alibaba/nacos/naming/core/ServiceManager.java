...
public class ServiceManager implements RecordListener<Service> {
	...
	public List<String> getAllServiceNameList(String namespaceId) {
		if (chooseServiceMap(namespaceId) == null) { return new ArrayList<>(); }
	...
	public void easyRemoveService(String namespaceId, String serviceName) throws Exception {
		...
		Service service = getService(namespaceId, serviceName);
		if (service == null) { throw new IllegalArgumentException("specified service not exist, serviceName : " + serviceName); }
	...
	public void registerInstance(String namespaceId, String serviceName, Instance instance) throws NacosException {
		//创建一个空的service（如果是第一次来注册实例，要先创建一个空service出来，放入注册表），此时不包含实例信息
	...
	public List<Instance> updateMetadata(String namespaceId, String serviceName, boolean isEphemeral, String action, boolean all, List<Instance> ips, Map<String, String> metadata) throws NacosException {
		...
		if (CollectionUtils.isEmpty(locatedInstance)) { throw new NacosException(NacosException.INVALID_PARAM, "not locate instances, input instances: " + ips); }
	...
	public List<Instance> getLocatedInstance(String namespaceId, String serviceName, boolean isEphemeral, boolean all, List<Instance> waitLocateInstance) throws NacosException {
		...
		if (datum == null) { throw new NacosException(NacosException.NOT_FOUND, "instances from consistencyService not exist, namespace: " + namespaceId + ", service: " + serviceName + ", ephemeral: " + isEphemeral); }
	...
	private Instance locateInstance(List<Instance> sources, Instance target) {
		...
		for (Instance element : sources) {
			...
			if (Objects.equals(element, target) && Objects.equals(element.getClusterName(), target.getClusterName())) { return element; }
		}
		return null;
	}
	...
	public Instance getInstance(String namespaceId, String serviceName, String cluster, String ip, int port) {
		...
		for (Instance instance : ips) {
			if (instance.getIp().equals(ip) && instance.getPort() == port) { return instance; }
		}
	...
	public List<Instance> updateIpAddresses(Service service, String action, boolean ephemeral, Instance... ips) throws NacosException {
		...
		if (instanceMap.size() <= 0 && UtilsAndCommons.UPDATE_INSTANCE_ACTION_ADD.equals(action)) { throw new IllegalArgumentException("ip list can not be empty, service: " + service.getName() + ", ip list: " + JacksonUtils.toJson(instanceMap.values())); }
	...
	public int getPagedService(String namespaceId, int startPage, int pageSize, String param, String containedInstance, List<Service> serviceList, boolean hasIpCount) {
		...
		if (chooseServiceMap(namespaceId) == null) { return 0; }
