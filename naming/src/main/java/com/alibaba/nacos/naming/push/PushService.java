...
public class PushService implements ApplicationContextAware, ApplicationListener<ServiceChangeEvent> {
	...
	public List<Subscriber> getClients(String serviceName, String namespaceId) {
		...
		if (Objects.isNull(clientConcurrentMap)) { return null; }
	...
	public boolean canEnablePush(String agent) {
		if (!switchDomain.isPushEnabled()) { return false; }
		...
		if (ClientInfo.ClientType.JAVA == clientInfo.type && clientInfo.version.compareTo(VersionUtil.parseVersion(switchDomain.getPushJavaVersion())) >= 0) {
			...
			return true;
		} else if (ClientInfo.ClientType.CSHARP == clientInfo.type && clientInfo.version.compareTo(VersionUtil.parseVersion(switchDomain.getPushCSharpVersion())) >= 0) { return true; }
	...
	public class PushClient {
		...
		public String toString() {
			...
			sb.append("serviceName: ").append(serviceName).append(", clusters: ").append(clusters).append(", address: ").append(socketAddr).append(", agent: ").append(agent);
			return sb.toString();
		}
		...
		public boolean equals(Object obj) {
			if (!(obj instanceof PushClient)) { return false; }
	...
	private static byte[] compressIfNecessary(byte[] dataBytes) throws IOException {
		...
		if (dataBytes.length < maxDataSizeUncompress) { return dataBytes; }
		...
		public void run() {
			while (true) {
				...
				try {
					...
					if (ackEntry == null) { throw new IllegalStateException("unable to find ackEntry for key: " + ackKey + ", ack json: " + json); }
