...
public class NamingProxy {
	...
	public static byte[] getData(List<String> keys, String server) throws Exception {
		...
		if (result.ok()) { return result.getData().getBytes(); }
	...
	public static boolean syncData(byte[] data, String curServer) {
		...
		try {
			...
			if (HttpURLConnection.HTTP_NOT_MODIFIED == result.getCode()) { return true; }
	...
	public static String reqApi(String api, Map<String, String> params, String curServer) throws Exception {
		try {
			...
			if (result.ok()) { return result.getData(); }

			if (HttpURLConnection.HTTP_NOT_MODIFIED == result.getCode()) { return StringUtils.EMPTY; }
	...
	public static String reqApi(String api, Map<String, String> params, String curServer, boolean isPost) throws Exception {
		try {
			...
			if (isPost) {
				result = HttpClient.httpPost("http://" + curServer + EnvUtil.getContextPath() + UtilsAndCommons.NACOS_NAMING_CONTEXT + "/api/" + api, headers, params);
			} else {
				result = HttpClient.httpGet("http://" + curServer + EnvUtil.getContextPath() + UtilsAndCommons.NACOS_NAMING_CONTEXT + "/api/" + api, headers, params);
			}
			...
			throw new IOException("failed to req API:" + "http://" + curServer + EnvUtil.getContextPath() + UtilsAndCommons.NACOS_NAMING_CONTEXT + "/api/" + api + ". code:" + result.getCode() + " msg: " + result.getMessage());
		} catch (Exception e) {
			Loggers.SRV_LOG.warn("NamingProxy", e);
		}
		return StringUtils.EMPTY;
