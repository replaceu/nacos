...
public class RaftPeerSet extends MemberChangeListener implements Closeable {
	...
	public RaftPeer getLeader() {
		if (EnvUtil.getStandaloneMode()) { return local(); }
		return leader;
	}
	...
	public RaftPeer decideLeader(RaftPeer candidate) {
		...
		//如果最大的投票数大于等于集群节点数的一半加一，则设置该节点状态为Leader
		if (maxApproveCount >= majorityCount()) {
			RaftPeer peer = peers.get(maxApprovePeer);
			peer.state = RaftPeer.State.LEADER;
			//如果leader并不是选出来的节点peer，则把选出来的节点设置为leader节点，并进行通知
	...
	public RaftPeer local() {
		...
		if (peer == null) { throw new IllegalStateException("unable to find local peer: " + NetUtils.localServer() + ", all peers: " + Arrays.toString(peers.keySet().toArray())); }
